...
from Utils import get_optimizer, parse_args, os
...
def init_train_and_validate(model, trainer, args, train_loader, val_loader):
    ...
    trainer.train(model, train_loader, optimizer,  args.epochs, args.gradient_clipping)
    test_loss = trainer.test(model, val_loader)
    return test_loss
...
def main():
    ...
    model_path = 'lstm_ae_toy_model.pth'
